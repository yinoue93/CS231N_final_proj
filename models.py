...
class Unet(object):
	...
	def create_model(self):
		...
		for layerName in self.config.layer_keys:
			...
			currLayer = conv_relu(currLayer, layerParams, is_train=self.is_train, name=layerName)
			self.layers[layerName] = currLayer

		self.layers['output'] = currLayer
	...
	def add_loss_op(self):
		# regression loss
		self.loss_op = tf.nn.l2_loss(self.layers['output']-self.output_placeholder)
